document.addEventListener('DOMContentLoaded', function() {
    ...
    (function initCounselorInfoFlow(){
        ...
        // Initialize PSGC address dropdowns when modal is first shown (safe to call multiple times)
        modalEl.addEventListener('shown.bs.modal', function () {
            if (window.PsgcAddress) {
                window.PsgcAddress.init('c_info_');
            }
        });
        ...
        submitBtn.addEventListener('click', function(e){
            ...
            const firstname = (getField('c_info_firstname')?.value || '').trim();
            const lastname  = (getField('c_info_lastname')?.value || '').trim();
            const mi        = (getField('c_info_mi')?.value || '').trim();
            ...
            if (!counselor_id || !firstname || !lastname || !degree || !email || !contact_number || !address) {
                return showWarn('Please fill in all required fields (First Name, Last Name, and address selection are required).');
            }
            ...
            .then(data => {
                ...
                if (data.status === 'success') {
                    ...
                    // Clear fields
                    ['c_info_firstname','c_info_lastname','c_info_mi','c_info_degree','c_info_email','c_info_contact','c_info_address','c_info_civil_status','c_info_sex','c_info_birthdate']
                        .forEach(function(id){ const el = getField(id); if (el) el.value = ''; });
                    // Reset PSGC dropdowns and street
                    ['c_info_region','c_info_province','c_info_city','c_info_barangay'].forEach(function(id){
                        const el = getField(id);
                        if (el) { el.selectedIndex = 0; el.disabled = (id !== 'c_info_region'); }
                    });
                    const streetEl = getField('c_info_street');
    ...
    // Function to update the password strength meter display
    function updatePasswordStrengthMeter(password) {
        const { level } = calculatePasswordStrength(password);
        
        // Reset classes
        strengthMeterFill.classList.remove('weak', 'moderate', 'strong');
        ...
        // Show the meter immediately when any character is typed
        ...
        switch (level) {
            case 'weak':
                ...
                strengthDescription.textContent = strengthDescription.dataset.weak;
                break;
            case 'moderate':
                strengthMeterFill.classList.add('moderate');
                strengthLabel.classList.add('moderate');
                strengthLabel.textContent = 'Moderate';
                strengthDescription.textContent = strengthDescription.dataset.moderate;
                break;
            case 'strong':
                strengthMeterFill.classList.add('strong');
                strengthLabel.classList.add('strong');
                strengthLabel.textContent = 'Strong';
                strengthDescription.textContent = strengthDescription.dataset.strong;
                break;
        }
    }
    
    // Add event listener for real-time password strength checking
    if (signUpPasswordInput) {
        signUpPasswordInput.addEventListener('input', function() {
            updatePasswordStrengthMeter(this.value);
        });
        
        // Also update when modal is closed (to reset)
    ...
    document.getElementById('signUpRole').addEventListener('change', function() {
        ...
        const signUpUserIdInput = document.getElementById('signUpUserIdInput');
        if (selectedRole === 'student') {
            signUpUserIdInput.placeholder = 'Preferred User ID (exactly 10 digits)';
            ...
            signUpUserIdInput.setAttribute('minlength', '10');
        } else if (selectedRole === 'counselor') {
